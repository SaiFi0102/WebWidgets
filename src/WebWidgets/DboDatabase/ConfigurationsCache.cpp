...
std::shared_ptr<ConfigurationBoolData> ConfigurationsCache::GetBoolPtr(const std::string &Name, long long ModuleId) const
{
	BoolMaps::const_iterator itr = BoolMap.find(std::make_pair(ModuleId, Name));
	if(itr == BoolMap.end())
	{
		return std::shared_ptr<ConfigurationBoolData>();
	}
	return itr->second;
}

std::shared_ptr<ConfigurationDoubleData> ConfigurationsCache::GetDoublePtr(const std::string &Name, long long ModuleId) const
{
	DoubleMaps::const_iterator itr = DoubleMap.find(std::make_pair(ModuleId, Name));
	if(itr == DoubleMap.end())
	{
		return std::shared_ptr<ConfigurationDoubleData>();
	}
	return itr->second;
}

std::shared_ptr<ConfigurationEnumData> ConfigurationsCache::GetEnumPtr(const std::string &Name, long long ModuleId) const
{
	EnumMaps::const_iterator itr = EnumMap.find(std::make_pair(ModuleId, Name));
	if(itr == EnumMap.end())
	{
		return std::shared_ptr<ConfigurationEnumData>();
	}
	return itr->second;
}

std::shared_ptr<ConfigurationFloatData> ConfigurationsCache::GetFloatPtr(const std::string &Name, long long ModuleId) const
{
	FloatMaps::const_iterator itr = FloatMap.find(std::make_pair(ModuleId, Name));
	if(itr == FloatMap.end())
	{
		return std::shared_ptr<ConfigurationFloatData>();
	}
	return itr->second;
}

std::shared_ptr<ConfigurationIntData> ConfigurationsCache::GetIntPtr(const std::string &Name, long long ModuleId) const
{
	IntMaps::const_iterator itr = IntMap.find(std::make_pair(ModuleId, Name));
	if(itr == IntMap.end())
	{
		return std::shared_ptr<ConfigurationIntData>();
	}
	return itr->second;
}

std::shared_ptr<ConfigurationLongIntData> ConfigurationsCache::GetLongIntPtr(const std::string &Name, long long ModuleId) const
{
	LongIntMaps::const_iterator itr = LongIntMap.find(std::make_pair(ModuleId, Name));
	if(itr == LongIntMap.end())
	{
		return std::shared_ptr<ConfigurationLongIntData>();
	}
	return itr->second;
}

std::shared_ptr<ConfigurationStringData> ConfigurationsCache::GetStringPtr(const std::string &Name, long long ModuleId) const
{
	StringMaps::const_iterator itr = StringMap.find(std::make_pair(ModuleId, Name));
	if(itr == StringMap.end())
	{
		return std::shared_ptr<ConfigurationStringData>();
	}
	return itr->second;
}
...
{
	std::shared_ptr<ConfigurationBoolData> BoolPtr = GetBoolPtr(Name, ModuleId);
	if(!BoolPtr)
	{
...
{
	std::shared_ptr<ConfigurationDoubleData> DoublePtr = GetDoublePtr(Name, ModuleId);
	if(!DoublePtr)
	{
...
{
	std::shared_ptr<ConfigurationEnumData> EnumPtr = GetEnumPtr(Name, ModuleId);
	if(!EnumPtr)
	{
...
{
	std::shared_ptr<ConfigurationFloatData> FloatPtr = GetFloatPtr(Name, ModuleId);
	if(!FloatPtr)
	{
...
{
	std::shared_ptr<ConfigurationIntData> IntPtr = GetIntPtr(Name, ModuleId);
	if(!IntPtr)
	{
...
{
	std::shared_ptr<ConfigurationLongIntData> LongIntPtr = GetLongIntPtr(Name, ModuleId);
	if(!LongIntPtr)
	{
...
{
	std::shared_ptr<ConfigurationStringData> StringPtr = GetStringPtr(Name, ModuleId);
	if(!StringPtr)
	{
